...
int main(int argc, char *argv[]){	
  ...
  while(run){
	...
	++npck;
	/* Decode the packet */
	if(pckStruct_p=depackLOS(pck,n,&nData)){
	  short dummy;
	  /* Check crc */
	  wrapperLOS(pck,n,&dummy,&crcOld);
	  crcNew=crc_short(pckStruct_p,(nData+nData%2)/2);
	  ...
	  //if(!mypasue) mvwprintw(statwin,18,1,"%0x %0x",crcOld,crcNew);
	  //if(!mypasue) mvwprintw(statwin,18,1,"%d",pcktime);
	  ++ntype[type];
	  totBytes+=n;
	  burstBytes+=n;
	  currBytes+=n;
    ...
    if(!mypasue) wnoutrefresh(dumpwin);  /* Show scroll if not paused */
