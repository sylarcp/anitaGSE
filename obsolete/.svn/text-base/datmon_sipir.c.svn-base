/*  Copyright (C) 2005 Predrag Miocinovic <predrag@phys.hawaii.edu> 
  ...
  implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.
   ...
   ANITA slow TDRSS data pipe monitoring tool   
...
int main(int argc, char *argv[]){	
  ...
  while(run){
	...
	++npck;
	/* Decode the packet */
	if(pckStruct_p=depackSIPlr(pck,n,&nData)){
	  unsigned char dummy;
	  /* Check crc */
	  wrapperSIPlr(pck,n,&dummy,&src);
	  type=pckType(pckStruct_p);
	  // Format unknown at the moment, so time can't be extracted
